...
data = xlsread('Steady-State CSTR Data.xlsx','RandomBias+Gaussian Noise');
data = data(:,2:end);
...
nlcon = @(x)InvProbV2Cons(x,Imat_t,tn,ni,nh,no,at_C_in_t,at_H_in_t,at_O_in_t,fv_in_t,V,CAf_in_t,CBf_in_t,CDf_in_t,mol_W_in_t,delta,data);
...
a = w_sol(ni*nh+nh*no+nh+no+no*tn+1:ni*nh+nh*no+nh+no+no*tn+no);
